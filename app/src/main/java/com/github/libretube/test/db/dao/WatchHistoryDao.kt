...
interface WatchHistoryDao {
    @Query("SELECT * FROM watchHistoryItem ORDER BY watchedAt DESC")
    suspend fun getAll(): List<WatchHistoryItem>

    @Query("SELECT * FROM watchHistoryItem ORDER BY watchedAt DESC LIMIT :limit OFFSET :offset")
    ...
    @Query("SELECT * FROM watchHistoryItem ORDER BY watchedAt ASC LIMIT 1")
